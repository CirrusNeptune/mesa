...
ir_vertex_attributes_visitor::ir_vertex_attributes_visitor(struct pipe_vertex_element elements[16])
   : elements(elements)
{
}
...
ir_vertex_attributes_visitor::visit(ir_variable *ir)
{
   if (ir->data.mode == ir_var_shader_in) {
      unsigned int location = (unsigned int)ir->data.location - VERT_ATTRIB_GENERIC0;
      assert(location < 16u);
      ...
      if (ir->type->is_float()) {
         format = (enum pipe_format)((int)PIPE_FORMAT_R32_FLOAT + ir->type->vector_elements);
      } else if (ir->type->base_type == GLSL_TYPE_INT) {
         format = (enum pipe_format)((int)PIPE_FORMAT_R32_SINT + ir->type->vector_elements);
      } else if (ir->type->base_type == GLSL_TYPE_UINT) {
         format = (enum pipe_format)((int)PIPE_FORMAT_R32_UINT + ir->type->vector_elements);
      } else {
         assert(0);
      }

      struct pipe_vertex_element *element = &elements[location];
      element->src_offset = location;
